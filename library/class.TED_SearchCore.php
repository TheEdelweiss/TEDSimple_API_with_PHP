...
public function __construct($materials, $query) 
{
...
function searchResult($materials, $keywords) 
{
  ...
  {      		
    ...
    {		
	  ...
	  {
		$material[$keysArray[$i]] = 
		   htmlspecialchars(strip_tags($material[$keysArray[$i]]));	
	  }
    }
  ...
  {
    $this -> JSONoutput[0]["error_respose"] = "Nici o coincidenta in BD !";
  }
...
private function removeWordEndingsRO($word) 
{
  if(is_string($word))
  {		
...
private function cutStopWordsRO($query) 
{		
  if (is_string($query)) 
  {	
...
private function secureQueryString($query)
{
...
private function explodeQuery ($query) 
{
